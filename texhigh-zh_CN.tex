...
\documentclass[class=article,openany]{cusdoc}[2025/10/10]
\usepackage{xurl}
...
\ekeys_cmd_new_scanner:nnnpn { maybe-a-string } { 2 }
  {
    \ekeys_cmd_register_cs:c { #1-arg#3-maybe-a-string-default }
    \tl_set:ce { #1-arg#3-maybe-a-string-default }
      { \tl_if_blank:nTF {#2} { \c_novalue_tl } { \exp_not:n {#2} } }
  }
  {
    \peek_meaning:NTF \c_space_token
      {
        ...
        \ekeys_cmd_add_arg:v
          { \ekeys_cmd_name: -arg \ekeys_cmd_args: -maybe-a-string-default }
      ...
      }
      {
        \peek_catcode:NTF \c_group_begin_token
          { 
            ...
            \__this_maybe_a_string:n
          }
          { \__this_maybe_a_string_check:N }
      }
  }
\cs_new:Npn \__this_maybe_a_string_check:N #1
  {
    \bool_lazy_and:nnTF
      { ! \token_if_control_word_p:N #1 }
      { ! \token_if_control_symbol_p:N #1 }
      {
        \str_if_in:nnTF 
      ...
      { \use_ii:nn }
        {
          \ekeys_cmd_add_arg:n { \BooleanTrue }
          \exp_args:No \__this_maybe_a_string:n { \token_to_str:N #1 }
        }
        {
          \ekeys_cmd_add_arg:n { \BooleanFalse }
          \ekeys_cmd_add_arg:v
...
\cs_new:Npn \__this_maybe_a_string:n #1
  {
    \ekeys_cmd_add_arg:n {#1}
    \ekeys_cmd_scanner_end:
  }
\ekeysdeclarecmd\escape{m K{{maybe-a-string}}}
  {\texttt{\textbackslash #1\IfValueT{#3}{\IfBooleanTF{#2}{#3}{\{#3\}}}}}
\ekeys_cmd_new_scanner:nnnpn { collect-n-or-arg } { 2 }
  {
    \ekeys_cmd_register_cs:c { #1-arg#3-collect-n-or-arg:w }
    \tl_set:Ne \l_tmpa_tl { \int_eval:n { \use:n #2 + 0 } }
    \use:e
      {
        \cs_generate_from_arg_count:NNnn
          \exp_not:c { #1-arg#3-collect-n-or-arg:w }
          \cs_new:Npn
          { \l_tmpa_tl }
          { {
            \if_case:w \l_tmpa_tl \exp_stop_f:
            \or: \exp_not:n {##1}
            \or: \exp_not:n {##1##2}
            \or: \exp_not:n {##1##2##3}
    ...
    \peek_remove_spaces:n
      {
        \peek_catcode:NTF \c_group_begin_token
          ...
          {
            \ekeys_cmd_add_arg:n { \BooleanTrue }
            \exp_args:NNc \exp_after:wN \__this_collect_n:n
              { \ekeys_cmd_name:-arg\ekeys_cmd_args:-collect-n-or-arg:w }
          }
      }
  }
\cs_new:Npn \__this_collect_n:n #1
  {
    \ekeys_cmd_add_arg:n {#1}
    \ekeys_cmd_scanner_end:
  }
\ekeysdeclarecmd \__this_collect_x:w { K{{collect-n-or-arg}{2}} }
  { \texttt{\textbackslash x\IfBooleanTF{#1}{#2}{\{#2\}}} }
\ekeysdeclarecmd \__this_collect_u:w { K{{collect-n-or-arg}{4}} }
  { \texttt{\textbackslash u\IfBooleanTF{#1}{#2}{\{#2\}}} }

\newlist{regexsyntax}{description}{1}
\setlist[regexsyntax]{nosep,
  leftmargin=1cm,labelwidth=8mm,labelsep=2mm,itemindent=5mm,align=left,
  before={\let\V\Verbatimize \let\x\__this_collect_x:w \let\u\__this_collect_u:w},
}
...
\changes{v0.4.1-a}{2025/10/04}{把部分 \cs{char_generate:nn} 替换为 \cs[break-at-any]{codepoint_generate:nn}。}
\docchange{v0.4.0}{2025/09/24}{添加文档。}
\begin{document}
...
\begin{function}{\THSetCharReplacement}
  \begin{syntax}
    ...
    \V\THSetCharReplacement * \marg{char} \marg{repr}
  \end{syntax}
设置 \meta{char} 的替换文本，在显示 \meta{char} 时，会替换为 \meta{repr}。
...
\docchange{v0.4.1-a}{2025/10/07}{更正 \cs{THCollectRange} 的说明。}
\begin{function}{\THCollectRange}
  ...
  \end{syntax}
收集普通代码段或逃逸代码段。\meta{do code} 可以使用 3 个参数，
...
\docchange{v0.4.1-a}{2025/09/29}{修复 \pkg{tcolorbox} 的选项的路径。}
...
\docchange{v0.4.1-a}{2025/10/08}{补充正则表达式的语法。}
...
\begin{regexsyntax}
  \item[\texttt{.}] 除了 \escape{n}（U+0A）外的任何字符，如果使用 \texttt{s} 标签（flag）则可以会包含此字符。
  ...
  \item[\escape p{\meta{class}}] 名为 \meta{class} 的 Unicode 字符类。
  \item[\escape PX] 不属于 Unicode 字符类 \texttt{X} 的字符。
  \item[\escape P{\meta{class}}] 不属于 \meta{class} 字符类的字符。
\end{regexsyntax}
...
\begin{regexsyntax}
  ...
  \item[\texttt{(?P<\meta{name}>\meta{exp})}] 命名（同时也编号）捕获组，\meta{name} 只能由字母、数字或 \texttt{\V{._[]}} 组成。
  \item[\texttt{(?<\meta{name}>\meta{exp})}] 命名（同时也编号）捕获组，\meta{name} 只能由字母、数字或 \texttt{\V{._[]}} 组成。
  \item[\texttt{(?:\meta{exp})}] 非捕获组。
  \item[\texttt{?\meta{flags}}] 设置标签（在当前组中有效）。\texttt{i} 标签可启用大小写不敏感的匹配。
  \item[\texttt{(flags:\meta{exp})}] 设置标签（在 \meta{exp} 中有效）。
\end{regexsyntax}

捕获组的名称只能由字母、数字或 \texttt{\Verbatimize{._[]}} 组成。
且必须以 \texttt{\_} 或字母开头。
...
\begin{regexsyntax}\def\p{\escape p}
  \item[\texttt{\textbackslash \meta{char}}] 匹配字面值 \meta{char}。\meta{char} 不能是 ASCII 以外的字符，也不能是 \texttt{0-9A-Za-z<>} 之一，也不能是以下列出的其它转义字符（它们有特殊含义）。
  \item[\escape a] 表示 \x 07。
  ...
  % \item[\123]
  \item[\x hh] 16 进制数 \veta{hh} 表示的字符。
  \item[\x{\meta{hex num}}] 16 进制数 \meta{hex num} 表示的字符。
  \item[\u hhhh] 16 进制数 \veta{hhhh} 表示的字符。
  \item[\u{\meta{hex num}}] 16 进制数 \meta{hex num} 表示的字符。
  % \item[\U hhhhhhhh]
  % \item[\U{hex num}]
  \item[\escape p{\meta{class}}] Unicode 字符类 \meta{class}。
  \item[\escape P{\meta{class}}] 非 Unicode 字符类 \meta{class}。
...
\escape p{\meta{class}} 和 \escape P{\meta{class}} 中的 \meta{class} 是某个 Unicode 字符类，
