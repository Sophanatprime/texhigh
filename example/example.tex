...
\texhighverb|\THSetCS| 改变显示效果。可以为它们设置前景色、背景色，
...
\makeatother
\THSetCS[]{tikzcs}{\mbox{\THcolor{blue}\bfseries #1#2}}
...
\THSetRange{mycomment}
  ...
  {% \appendpercent “参数处理器”把 \THmP（即“%”）放到参数的前面
    ...
    \THSetCH[]{catcode.14}{\firstComment{#1}}%
...
\THSetRange * {change-at} % 带 * 的不会自动加上 \begingroup \endgroup
  ...
  [ \ensuremath{\rangle} ]
\THSetCS[]{atat-i}{ \__this_atat_cs_internal:nn {#1} {#2} }
\THSetCS[]{atat-p}{ \__this_atat_cs_public:nn {#1} {#2} }
\ExplSyntaxOff
...
\IfFileExists{./texhigh.prelude.ths.tex}
  {\def\texhighfilename{./texhigh.prelude.ths.tex}}
  {\IfFileExists{../texhigh.prelude.ths.tex}
    {\def\texhighfilename{../texhigh.prelude.ths.tex}}
    {\IfFileExists{./texhigh.prelude.ths}
      {\def\texhighfilename{./texhigh.prelude.ths}}
      {\def\texhighfilename{../texhigh.prelude.ths}}}}
\texhighfile[use-ctab=latex3code]{\texhighfilename}
